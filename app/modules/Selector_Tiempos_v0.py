...
def Tiempo(Tarea, var1, valor1):
    ...
    valores_multiplicados['Tarea'] = Tarea
    valores_multiplicados['Tiempo_s'] = Tiempo_s
    valores_multiplicados['Tiempo_m'] = Tiempo_m

    # Renombrar solo las columnas que tienen el sufijo '_s'
    renombrar_columnas = {col: col.replace('_s', '') for col in columnas}
    valores_multiplicados.rename(columns=renombrar_columnas, inplace=True)
    
    # Definir el orden final de las columnas (manteniendo 'Tiempo_s' sin cambios)
    columnas_finales = ['Tarea', 'Tiempo_s', 'Tiempo_m'] + list(renombrar_columnas.values())
